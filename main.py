...
async def start(event):
    ...
    async with bot.conversation(event.chat_id) as x:
        buttons = [
            ...
            [
                Button.inline("تسجيل جلسة بأيروجرام", data="pyrogram"),
        ...
        await event.reply("**- مرحبًـا بك في بوت جلب روابط المميز من حساباتك المسجلة 🔗**\n\n- اختر من الازرار ادناه ما تود فعله.", buttons=buttons)
...
async def start_lis(event):
    ...
    if data == "pyrogram":
        async with bot.conversation(event.chat_id) as x:
            await x.send_message("⋙ أرسل الآن سيشن جلسة بأيرورجرام")
            txt = await x.get_response()
            session = txt.text
            try:
                Convert_sess = MangSession.PYROGRAM_TO_TELETHON(session)
            except:
                return await x.send_message("⋙ أرسل سيشن بأيروجرام بشكل صحيح")
    ...
    if data == "telethon":
        async with bot.conversation(event.chat_id) as x:
            await x.send_message("⋙ أرسل الآن سيشن جلسة تليثون")
            txt = await x.get_response()
            session = txt.text
            data = {"phone_number": "لم يتم التعرف", "two-step": "لا يوجد", "session": Convert_sess}
            acc = db.get("accounts")
    ...
    if data == "back" or data == "cancel":
        ...
        await event.edit("**- مرحبًـا بك في بوت جلب روابط المميز من حساباتك المسجلة 🔗**\n\n- اختر من الازرار ادناه ما تود فعله.", buttons=buttons)
    if data == "add":
        async with bot.conversation(event.chat_id) as x:
            await x.send_message("✔️الآن ارسل رقمك مع رمز دولتك , مثال :+201000000000")
            txt = await x.get_response()
            ...
            try:
                ...
                try:
                    ...
                    await x.send_message("الباسورد الذي أرسلته غير صالح.")
                    return
                string_session = app.session.save()
    ...
    if data == "get_gift":
        await event.answer(f"⋙ تم بدا جلب روابط المميز من الحسابات بالرجاء انتظار اشعار", alert=True)
        acc = db.get("accounts")
        count = 0
        for i in acc:
        ...
        await client.send_message(admin, f"⋙ تم الانتهاء من فحص الحسابات تم إيجاد {count} روابط")
    if data == "join_channel":
        async with bot.conversation(event.chat_id) as x:
            await x.send_message("⋙ أرسل الآن رابط او معرف القناة التي تريد الانضمام لها بكل الحسابات")
            ch = await x.get_response()
            if "@" not in ch.text:
                if "/t.me/" not in ch.text:
                    await x.send_message(f"⋙ أرسل رابط او معرف القناة بشكل صحيح")
                    return 
    ...
    if data == "leave_channel":
        async with bot.conversation(event.chat_id) as x:
            await x.send_message("⋙ أرسل الآن رابط او معرف القناة التي تريد المغادرة منها بكل الحسابات")
            ch = await x.get_response()
            if "@" not in ch.text:
                if "/t.me/" not in ch.text:
                    await x.send_message(f"⋙ أرسل رابط او معرف القناة بشكل صحيح")
    ...
    if data == "leave_all":
        ...
        await event.edit("**- هل تود فعلًا تأكيد المغادرة من كل الحسابات؟**", buttons=buttons)
    ...
    if data == "get_session":
        async with bot.conversation(event.chat_id) as x:
            await x.send_message("⋙ أرسل الآن رقم الهاتف الذي قمت بتسجيله للبوت لجلب السيشن منه")
            txt = await x.get_response()
            phone_number = txt.text.replace("+", "").replace(" ", "")
            acc = db.get("accounts")
            for i in acc:
                if phone_number == i['phone_number']:
                    ...
                    return
            await x.send_message("- لم يتم العثور على هذا الرقم ضمن قائمة الحسابات")
        ...
        try:
            if event.media and event.media.document:
                ...
                if olddb.exists("sessions") and len(olddb.get("sessions")) > 0:
                    ...
                    await x.send_message(f'تم بنجاح إضافة {len(olddb.get("sessions"))} حساب إلى خزن البوت الحإلى.')
                else: 
                    await x.send_message(f'• هذا الخزن لا يحتوي على أي ارقام')
